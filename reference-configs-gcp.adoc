...
. 磁盘限制可以防止您单独使用磁盘来达到最大系统容量限制。您可以通过达到容量限制 https://["将非活动数据分层到对象存储"^]。
+
...
. 有关 Google Cloud Platform 区域支持，请参见 https://["Cloud Volumes 全球地区"^]。
...
** n2-standard-64
+
在部署符合条件的实例类型时、您可以启用_Flash Cache_和高写入速度。要启用8、896字节的最大传输单元、必须在部署中选择VPC-1、VPC-2或VPC-3。MTU越高、网络吞吐量就越高。有关启动其中一种部署的详细信息、请参见 https://["在Google Cloud中启动HA对"]。
