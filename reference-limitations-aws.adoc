...
+
您可以在从BlueXP创建卷时选择无存储效率、也可以先创建卷、然后再创建卷 http://["使用命令行界面禁用数据压缩"^]。
...
Cloud Volumes ONTAP 不会在空闲时释放 CPU ，因此 https://["Amazon CloudWatch"^] 可以为 EC2 实例报告 CPU 使用率较高的警告，因为它会看到 100% 的使用率。您可以忽略此警报。ONTAP statistics 命令可显示 CPU 的实际使用情况。
...
https://["了解存储在 AWS 中运行的 Cloud Volumes ONTAP HA 对中的工作原理"^]。
